...
def maybe_take_sleep_tempo_choice(
    self,
    battle: Battle,
    ordered: List[Tuple[str, float]],
    chosen_choice: str,
    ...
    def _alive_counts() -> tuple[int, int]:
        my_alive = sum(
            1
            for mon in (getattr(battle, "team", {}) or {}).values()
            if mon is not None and not getattr(mon, "fainted", False)
        ...
        opp_alive = sum(1 for mon in opp_team.values() if mon is not None and not getattr(mon, "fainted", False))
        opp_fainted = sum(1 for mon in opp_team.values() if mon is not None and getattr(mon, "fainted", False))
        return max(1, my_alive), max(1, max(opp_alive, 6 - opp_fainted))

    turn = int(getattr(battle, "turn", 0) or 0)
    my_alive, opp_alive = _alive_counts()
    min_turn = int(getattr(self, "SLEEP_TEMPO_MIN_TURN", 12))
    max_my_alive = int(getattr(self, "SLEEP_TEMPO_MAX_MY_ALIVE", 2))
    max_opp_alive = int(getattr(self, "SLEEP_TEMPO_MAX_OPP_ALIVE", 3))
    if turn < min_turn or my_alive > max_my_alive or opp_alive > max_opp_alive:
        _record(
            "not_endgame",
            turn=int(turn),
            min_turn=int(min_turn),
            my_alive=int(my_alive),
            opp_alive=int(opp_alive),
            max_my_alive=int(max_my_alive),
            max_opp_alive=int(max_opp_alive),
        )
        return chosen_choice
    ...
    chosen_weight = weights.get(chosen_choice, 0.0)
    best_sleep_choice = ""
    best_sleep_weight = 0.0
    best_sleep_score = 0.0
    best_sleep_accuracy = 0.0
    ...
    for choice, weight in candidate_choices:
        ...
        if status_type not in {"sleep", "yawn"}:
            continue
        if self._status_choice_is_obviously_bad(battle, move, active, opponent):
            continue
        raw_accuracy = getattr(move, "accuracy", None)
        accuracy = 1.0 if raw_accuracy in (None, True) else max(0.0, min(1.0, float(raw_accuracy or 0.0) / 100.0))
        if accuracy < 0.70:
            continue
        status_score = float(self._should_use_status_move(move, active, opponent, battle) or 0.0)
        candidate_weight = float(weight or weights.get(choice, 0.0) or 0.0)
        key = (candidate_weight, status_score, accuracy)
        best_key = (best_sleep_weight, best_sleep_score, best_sleep_accuracy)
        if not best_sleep_choice or key > best_key:
            best_sleep_choice = choice
            best_sleep_weight = candidate_weight
            best_sleep_score = status_score
            best_sleep_accuracy = accuracy

    if not best_sleep_choice:
        _record("no_sleep_candidate", turn=int(turn), my_alive=int(my_alive), opp_alive=int(opp_alive))
        return chosen_choice

    min_ratio = float(getattr(self, "SLEEP_TEMPO_MIN_POLICY_RATIO", 0.45))
    max_drop = float(getattr(self, "SLEEP_TEMPO_MAX_SCORE_DROP", 0.20))
    policy_ratio = 1.0 if chosen_weight <= 0.0 else best_sleep_weight / max(chosen_weight, 1e-6)
    score_drop = max(0.0, chosen_weight - best_sleep_weight)
    if policy_ratio < min_ratio or score_drop > max_drop:
        _record(
            "policy_ratio",
            sleep_choice=best_sleep_choice,
            chosen_weight=float(chosen_weight),
            sleep_weight=float(best_sleep_weight),
            policy_ratio=float(policy_ratio),
            min_policy_ratio=float(min_ratio),
            score_drop=float(score_drop),
            max_score_drop=float(max_drop),
    ...
    opp_has_recovery = bool(known_moves & (self.RECOVERY_MOVES | self.PROTECT_MOVES))
    if best_damage_score >= ko_threshold and not opp_has_recovery:
        _record(
            "ko_guard",
            sleep_choice=best_sleep_choice,
            opp_hp=float(opp_hp),
        ...
        return chosen_choice
    if not (opp_has_recovery or active_hp <= 0.35 or opp_hp <= 0.35 or my_alive < opp_alive):
        _record(
            "no_tempo_need",
            sleep_choice=best_sleep_choice,
            active_hp=float(active_hp),
            ...
            my_alive=int(my_alive),
            opp_alive=int(opp_alive),
            opp_has_recovery=bool(opp_has_recovery),
        )
        return chosen_choice
    ...
    _record(
        ...
        policy_ratio=float(policy_ratio),
        score_drop=float(score_drop),
        sleep_score=float(best_sleep_score),
        sleep_accuracy=float(best_sleep_accuracy),
        best_damage_score=float(best_damage_score),
        ko_threshold=float(ko_threshold),
        opp_has_recovery=bool(opp_has_recovery),
    )
    return best_sleep_choice
...
def select_move_from_results(
    ...
    def _return_choice(chosen_choice: str, path: str) -> str:
        ...
        if chosen_choice and bool(getattr(self, "SLEEP_TEMPO_GUARD_ENABLED", False)):
            chosen_choice, path = _apply_rerank_candidate(
                chosen_choice,
                path,
                self._maybe_take_sleep_tempo_choice,
            )
